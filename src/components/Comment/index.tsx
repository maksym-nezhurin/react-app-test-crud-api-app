...
export const Comment = ({ data, isAuthor }: IProps) => {
    ...
    const authorStyles = isAuthor ? 'bg-blue-100 text-blue-900 border-blue-500' : `bg-amber-500 text-white`;

    return (
        <div className={`${authorStyles} border-l-4 max-w-[300px] rounded-md p-4 flex flex-col justify-end`}>
            <div className="flex items-end justify-between">
                <span className="font-semibold">
                    {isAuthor ?
                        <span className="ml-2 px-2 py-1 text-xs rounded-full text-white bg-blue-500">Author</span> :
                        <span className={`ml-2 px-2 py-1 text-xs rounded-full text-white bg-green-500`}>Guest</span>
                    } {/* Badge for author */}
                </span>
