...
const NewsPage = () => {
  const axiosWrapper = new ApiService({ baseURL: `${API_URL}`, config: {} });
    ...
    <AnimatePresence>
      {
        ...
        >
          <header
            className={
              "flex-grow bg-gray-100 rounded-xl p-3 flex flex-col items-center justify-center justify-self-center px-8"
            }
          >
            <div className="text-center">
              <h1 className="text-2xl font-bold mb-4 text-brandRed">
                Welcome to News Page
              </h1>
              <p className="mb-6">
                The easiest way to create the creative images.
              </p>
            </div>

            <Select
              value={selectedCategory}
              onValueChange={(dat) => setCategory(dat)}
            >
          ...
          <main className={"h-full grid grid-rows-[auto_1fr]"}>
            ...
            <div
              ref={stickyRef}
              className={
                "flex flex-wrap gap-4 mt-6 justify-center overflow-y-scroll shadow-2xl py-4 max-h-[500px] p-6"
              }
            >
              {loading ? (
                <Spinner />
